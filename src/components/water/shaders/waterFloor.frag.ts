export const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    float t = sampleT(uv);
    if (uBlur > 0.0) {
      t += sampleT(uv + vec2( uBlur,  0.0));
      t += sampleT(uv + vec2(-uBlur,  0.0));
      t += sampleT(uv + vec2( 0.0,   uBlur));
      t += sampleT(uv + vec2( 0.0,  -uBlur));
      t *= 0.2;
    }
    ...
    if (rawScene > 0.9999) {
      ...
      shoreFade = smoothstep(0.0, uShoreDepth, abs(sceneD - waterD));
    }

    float alpha = mix(uDeepOpacity, 1.0, t) * uOpacity * fade * shoreFade;
